...
class HarmonicNeuralNetwork:
    ...
    def _sonify_layer_activity(self, layer_idx: int, activations: np.ndarray, 
        ...
        if description:  # Only print when called interactively
        ...
        for neuron_idx, activation in enumerate(activations_flat):
            ...
            if activation > 0.1:  # Only create sound for significant activations
        ...
        if description and np.any(layer_audio != 0):  # Only play when called interactively
...
def create_musical_composition(nn: HarmonicNeuralNetwork, duration: float = 5.0):
    ...
    for i, input_vector in enumerate(inputs):
        print(f"Generating segment {i+1}/{len(inputs)}")
    ...
    nn._play_audio(composition[:int(nn.sample_rate * 3)])  # Play first 3 seconds
